...
% Melis Orhun  - 4912071
% =============================================================
% Optimization algorithm with nonlinear inequality constraints
% and without min function
% *** Rudi ***
% =============================================================
% *** TABLE OF CONTENTS ***
%  
...
%  * fmincon with SQP and r=1
%  * fmincon with Interior-point and r=1
...
lmbda = 4;
rho_c = 33.5 + E1/3;
%rho_c = 32.5 + E1/3; % FOR QUESTION 5 ONLY
...
t_begin_sqp_0 = tic;
[r_sqp_0, out_sqp_0, exitflag_sqp_0, data_sqp_0] = fmincon(@cost_function,r0,[],[],[],[],lb,ub,@nlcon,options);
t_sqp_0 = toc(t_begin_sqp_0);
[rho_sqp_0, v_sqp_0, wr_sqp_0] = optimum_states(r_sqp_0);

% Interior Point with r = 0
options = optimoptions('fmincon','Display','iter','Algorithm','interior-point','MaxFunctionEvaluations',inf,'MaxIterations',inf); 
t_begin_ip_0 = tic;
[r_ip_0, out_ip_0, exitflag_ip_0, data_ip_0] = fmincon(@cost_function,r0,[],[],[],[],lb,ub,@nlcon,options);
...
options = optimoptions('fmincon','Display','iter','Algorithm','sqp'); % Use sqp
t_begin_sqp_099 = tic;
[r_sqp_099, out_sqp_099, exitflag_sqp_099, data_sqp_099] = fmincon(@cost_function,r0,[],[],[],[],lb,ub,@nlcon,options);
t_sqp_099 = toc(t_begin_sqp_099);
[rho_sqp_099, v_sqp_099, wr_sqp_099] = optimum_states(r_sqp_099);

% Interior Point with r = 0.99
options = optimoptions('fmincon','Display','iter','Algorithm','interior-point','MaxFunctionEvaluations',inf,'MaxIterations',inf);
t_begin_ip_099 = tic;
[r_ip_099, out_ip_099, exitflag_ip_099, data_ip_099] = fmincon(@cost_function,r0,[],[],[],[],lb,ub,@nlcon,options);
t_ip_099 = toc(t_begin_ip_099);
[rho_ip_099, v_ip_099, wr_ip_099] = optimum_states(r_ip_099);

% Set initial point r = 1
r0 = 1 * ones(121,1);

% SQP with r = 1
options = optimoptions('fmincon','Display','iter','Algorithm','sqp'); % Use sqp
t_begin_sqp_1 = tic;
[r_sqp_1, out_sqp_1, exitflag_sqp_1, data_sqp_1] = fmincon(@cost_function,r0,[],[],[],[],lb,ub,@nlcon,options);
t_sqp_1 = toc(t_begin_sqp_1);
[rho_sqp_1, v_sqp_1, wr_sqp_1] = optimum_states(r_sqp_1);

% Interior Point with r = 1
options = optimoptions('fmincon','Display','iter','Algorithm','interior-point','MaxFunctionEvaluations',inf,'MaxIterations',inf);
t_begin_ip_1 = tic;
[r_ip_1, out_ip_1, exitflag_ip_1, data_ip_1] = fmincon(@cost_function,r0,[],[],[],[],lb,ub,@nlcon,options);
t_ip_1 = toc(t_begin_ip_1);
[rho_ip_1, v_ip_1, wr_ip_1] = optimum_states(r_ip_1);
...
Tbl = table([out_sqp_0;out_ip_0;out_sqp_099;out_ip_099;out_sqp_1;out_ip_1;y],[t_sqp_0;t_ip_0;t_sqp_099;t_ip_099;t_sqp_1;t_ip_1;0], ...
          [data_sqp_0.iterations;data_ip_0.iterations;data_sqp_099.iterations;data_ip_099.iterations;data_sqp_1.iterations;data_ip_1.iterations;0], ...
          [exitflag_sqp_0;exitflag_ip_0;exitflag_sqp_099;exitflag_ip_099;exitflag_sqp_1;exitflag_ip_1;0], ...
          'VariableNames',{'Mini TTS (h)','Elapsed Time (s)','Iterations','ExitFlag'}, ...
          'RowNames',{'Rudi SQP (r=0)','Rudi IP (r=0)','Rudi SQP (r=0.99)','Rudi IP (r=0.99)','Rudi SQP (r=1)','Rudi IP (r=1)','No Control Case'});
disp(Tbl)

% % FOR QUESTION 5 ONLY: Display data of different algorithms for rho_c = 32.5
% Tbl = table([out_sqp_0;out_sqp_099;],[t_sqp_0;t_sqp_099], ...
%           [data_sqp_0.iterations;data_sqp_099.iterations], ...
%           [exitflag_sqp_0;exitflag_sqp_099], ...
%           'VariableNames',{'Mini TTS (h)','Elapsed Time (s)','Iterations','ExitFlag'}, ...
%           'RowNames',{'Rudi SQP (r=0)','Rudi SQP (r=0.99)'});
% %Tbl = table(Tbl,'VariableNames',{'rho_c = 32.5 + E1/3 and q0 = 7976 + E1*100'});
% Tbl = table(Tbl,'VariableNames',{'rho_c = 32.5 + E1/3 and rho_0 = 29.5'});
% disp(Tbl)
...
title("Ramp Metering Rate vs. Time (SQP with $r_{sp}(k)=0$)")
xlabel("Time (s)"); ylabel("Ramp Metering Rate"); ylim("padded"); grid minor

figure(5) % IP - density r=0.99
plot(0:10:1200,rho_sqp_099(1,1:121),0:10:1200,rho_sqp_099(2,1:121),0:10:1200,rho_sqp_099(3,1:121),0:10:1200,rho_sqp_099(4,1:121))
title("Traffic Density vs. Time for Each Lane (SQP with $r_{sp}(k)=0.99$)")
xlabel("Time (s)"); ylabel("Traffic Density [veh/(km.lane)]"); grid minor
legend('$\rho_1$(t)','$\rho_2$(t)','$\rho_3$(t)','$\rho_4$(t)')

figure(6) % IP - mean speed r=0.99
plot(0:10:1200,v_sqp_099(1,1:121),0:10:1200,v_sqp_099(2,1:121),0:10:1200,v_sqp_099(3,1:121),0:10:1200,v_sqp_099(4,1:121))
title("Mean Speed vs. Time (SQP with $r_{sp}(k)=0.99$)")
xlabel("Time (s)"); ylabel("Mean Speed [km/h]"); grid minor
legend('$v_1$(t)','$v_2$(t)','$v_3$(t)','$v_4$(t)','Location','northwest')

figure(7) % IP - number of vehicles in queue on the on-ramp r=0.99
plot(0:10:1200,wr_sqp_099) 
title(sprintf("Number of Vehicles in Queue on the On-ramp vs. Time\n(SQP with $r_{sp}(k)=0.99$)"))
xlabel("Time (s)"); ylabel("Queue Length [Veh]"); grid minor

figure(8) % IP - ramp metering rate r=0.99
plot(0:10:1200,r_sqp_099)
title("Ramp Metering Rate vs. Time (SQP with $r_{sp}(k)=0.99$)")
xlabel("Time (s)"); ylabel("Ramp Metering Rate"); ylim("padded"); grid minor

% Plot the optimized solutions for SQP - IP comparison r=0
figure(9) % density comparison
sgtitle(sprintf("Traffic Density Comparison\nfor SQP and IP Solutions of Rudi with $r_{sp}(k)=0$"),"FontSize",12,"FontWeight","bold")
%
subplot(2,2,1)
...
title(sprintf("Queue Length vs. Time\nfor SQP and IP Solutions of Rudi with $r_{sp}(k)=0.99$"))
xlabel("Time (s)"); ylabel("Queue Length [Veh]"); grid minor
legend('$w_{r,ip}$(t)','$w_{r,sqp}$(t)')

figure(12) % ramp metering rate comparison
plot(0:10:1200,r_ip_0,"--"); hold on; plot(0:10:1200,r_sqp_0,":"); 
title(sprintf("Ramp Metering Rate vs. Time\nfor SQP and IP Solutions of Rudi with $r_{sp}(k)=0$"))
xlabel("Time (s)"); ylabel("Ramp Metering Rate"); ylim("padded"); grid minor
legend('$r_{ip}$(t)','$r_{sqp}$(t)','Location','southeast')

% Plot the optimized solutions for SQP - IP comparison r=0.99
figure(13) % density comparison
sgtitle(sprintf("Traffic Density Comparison\nfor SQP and IP Solutions of Rudi with $r_{sp}(k)=0.99$"),"FontSize",12,"FontWeight","bold")
%
subplot(2,2,1)
plot(0:10:1200,rho_ip_099(1,1:121),"--"); hold on; plot(0:10:1200,rho_sqp_099(1,1:121),":");
title("Traffic Density vs. Time for Lane 1")
xlabel("Time (s)"); xlim([0,1200]); ylabel(sprintf("Traffic Density\n[veh/(km.lane)]")); grid minor
legend('$\rho_{1,ip}$(t)','$\rho_{1,sqp}$(t)')
%
subplot(2,2,2)
plot(0:10:1200,rho_ip_099(2,1:121),"--"); hold on; plot(0:10:1200,rho_sqp_099(2,1:121),":");
title("Traffic Density vs. Time for Lane 2")
xlabel("Time (s)"); xlim([0,1200]); ylabel(sprintf("Traffic Density\n[veh/(km.lane)]")); grid minor
legend('$\rho_{2,ip}$(t)','$\rho_{2,sqp}$(t)')
%
subplot(2,2,3)
plot(0:10:1200,rho_ip_099(3,1:121),"--"); hold on; plot(0:10:1200,rho_sqp_099(3,1:121),":");
title("Traffic Density vs. Time for Lane 3")
xlabel("Time (s)"); xlim([0,1200]); ylabel(sprintf("Traffic Density\n[veh/(km.lane)]")); grid minor
legend('$\rho_{3,ip}$(t)','$\rho_{3,sqp}$(t)')
%
subplot(2,2,4)
plot(0:10:1200,rho_ip_099(4,1:121),"--"); hold on; plot(0:10:1200,rho_sqp_099(4,1:121),":");
title("Traffic Density vs. Time for Lane 4")
xlabel("Time (s)"); xlim([0,1200]); ylabel(sprintf("Traffic Density\n[veh/(km.lane)]")); grid minor
legend('$\rho_{4,ip}$(t)','$\rho_{4,sqp}$(t)','Location','southwest')
set(gcf, 'Position',  [200, 100, 700, 500]);
...
%
subplot(2,2,1)
plot(0:10:1200,v_ip_099(1,1:121),"--"); hold on; plot(0:10:1200,v_sqp_099(1,1:121),":");
title("Mean Speed vs. Time for Lane 1")
xlabel("Time (s)"); xlim([0,1200]); ylabel(sprintf("Mean Speed [km/h]")); grid minor
legend('$v_{1,ip}$(t)','$v_{1,sqp}$(t)','Location','southeast')
%
subplot(2,2,2)
plot(0:10:1200,v_ip_099(2,1:121),"--"); hold on; plot(0:10:1200,v_sqp_099(2,1:121),":");
title("Mean Speed vs. Time for Lane 2")
xlabel("Time (s)"); xlim([0,1200]); ylabel(sprintf("Mean Speed [km/h]")); grid minor
legend('$v_{2,ip}$(t)','$v_{2,sqp}$(t)','Location','southeast')
%
subplot(2,2,3)
plot(0:10:1200,v_ip_099(3,1:121),"--"); hold on; plot(0:10:1200,v_sqp_099(3,1:121),":");
title("Mean Speed vs. Time for Lane 3")
xlabel("Time (s)"); xlim([0,1200]); ylabel(sprintf("Mean Speed [km/h]")); grid minor
legend('$v_{3,ip}$(t)','$v_{3,sqp}$(t)')
%
subplot(2,2,4)
plot(0:10:1200,v_ip_099(4,1:121),"--"); hold on; plot(0:10:1200,v_sqp_099(4,1:121),":");
title("Mean Speed vs. Time for Lane 4")
xlabel("Time (s)"); xlim([0,1200]); ylabel(sprintf("Mean Speed [km/h]")); grid minor
legend('$v_{4,ip}$(t)','$v_{4,sqp}$(t)')
set(gcf, 'Position',  [200, 100, 700, 500]);
...
plot(0:10:1200,wr_ip_099,"--"); hold on; plot(0:10:1200,wr_sqp_099,":"); 
title(sprintf("Queue Length vs. Time\nfor SQP and IP Solutions of Rudi with $r_{sp}(k)=0.99$"))
xlabel("Time (s)"); ylabel("Queue Length [Veh]"); grid minor
legend('$w_{r,ip}$(t)','$w_{r,sqp}$(t)')

figure(16) % ramp metering rate comparison
plot(0:10:1200,r_ip_099,"--"); hold on; plot(0:10:1200,r_sqp_099,":"); 
title(sprintf("Ramp Metering Rate vs. Time\nfor SQP and IP Solutions of Rudi with $r_{sp}(k)=0.99$"))
xlabel("Time (s)"); ylabel("Ramp Metering Rate"); ylim("padded"); grid minor
legend('$r_{ip}$(t)','$r_{sqp}$(t)','Location','southeast')

% Plot the optimized and no control solutions
figure(17) % density comparison
sgtitle(sprintf("Traffic Density Comparison\nfor No-Control and SQP-Optimized Solutions of Rudi"),"FontSize",12,"FontWeight","bold")
%
subplot(2,2,1)
...
figure(19) % queue length comparison
plot(0:10:1200,wr_no_control,"--"); hold on; plot(0:10:1200,wr_sqp_0,":"); 
title(sprintf("Queue Length vs. Time\nfor No-Control and SQP-Optimized Solutions of Rudi"))
xlabel("Time (s)"); ylabel("Queue Length [Veh]"); grid minor
...
function [c, ceq] = nlcon(r)
    ...
    %rho_c = 32.5 + E1/3; % veh/(km*lane) % FOR QUESTION 5 ONLY
    ...
    q = zeros(4,1);
    V = zeros(4,1);
    c = zeros(726,1); % Nonlinear inequality constraints
    ceq = []; % Nonlinear equality constraints

    % Initial input
    q0 = 8000 + 100*E1;
    %q0 = 7976 + 100*E1; % FOR QUESTION 5 ONLY

    % Initial states
    rho(1:4,1) = 30*ones(4,1);
    %rho(1:4,1) = 29.5*ones(4,1); % FOR QUESTION 5 ONLY
    v(1:4,1) = 80*ones(4,1);
    ...
    for j = 1:1:121
        ...
        rho(4,j+1) = rho(4,j) + (T/(lmbda*Li)) * (q(3)-q(4)+r(j)*Cr);
        ...
        wr(j+1) = wr(j) + T*(Dr-r(j)*Cr);

        c((j-1)*6+1,1) = -rho_c + rho(1,j+1);
        c((j-1)*6+2,1) = -rho_c + rho(2,j+1);
        c((j-1)*6+3,1) = -rho_c + rho(3,j+1);
        c((j-1)*6+4,1) = -rho_c + rho(4,j+1);
        c((j-1)*6+5,1) = (r(j) * Cr) - Cr*(rho_m-rho(4,j))/(rho_m-rho_c);
        c((j-1)*6+6,1) = (r(j) * Cr) - (Dr + (wr(j)/T));
...
% State update function for no control case
